...
contract AMMRouterTest is Test {
    ...
    function testConstructor() public {
        ...
        (,, address calculatedPair) = ammRouter.getReserves(address(tokenA), address(tokenB));
    ...
    function testGetReservesWithoutCreatingPair() public view {
        (uint256 reserveA_fromAB, uint256 reserveB_fromAB, address pair_fromAB) =
            ammRouter.getReserves(address(tokenA), address(tokenB));
        (uint256 reserveB_fromBA, uint256 reserveA_fromBA, address pair_fromBA) =
        ...
        address expectedPair = AMMLibrary.pairFor(
            address(pairFactory), address(tokenA), address(tokenB), pairFactory.INIT_CODE_PAIR_HASH()
        );
    ...
    function testAddLiquidity() public {
        ...
        assertEq(TokenPair(pairFromFactory).balanceOf(lpRecipient), expectedLpToProvider);
    ...
    function testIdenticalAddresses() public {
        ...
        ammRouter.addLiquidity(address(tokenA), address(tokenA), 1, 1, 0, 0, lp, _validDeadline());
    }

    function testZeroAddresses() public {
        vm.expectRevert(bytes("ZERO_ADDRESS"));
        ammRouter.addLiquidity(address(0), address(tokenA), 1, 1, 0, 0, lp, _validDeadline());
    }
    ...
    function testRemoveLiquidity() public {
        ...
        (uint256 reserveABefore, uint256 reserveBBefore,) = ammRouter.getReserves(address(tokenA), address(tokenB));

        assertEq(reserveABefore, amountADesired);
        assertEq(reserveBBefore, amountBDesired);

        uint256 beforeBurnBalanceAOfTokenPair = IERC20(address(tokenA)).balanceOf(pair);
        uint256 beforeBurnBalanceBOfTokenPair = IERC20(address(tokenB)).balanceOf(pair);
        ...
        uint256 expectedAmountARemoved = (liquidity * reserveABefore) / expectedTotalSupply;
        uint256 expectedAmountBRemoved = (liquidity * reserveBBefore) / expectedTotalSupply;

        // Act: approve LP tokens to router and remove all liquidity
        _safeApprove(pair, address(ammRouter), type(uint256).max);

        (uint256 amountARemoved, uint256 amountBRemoved) = ammRouter.removeLiquidity(
            address(tokenA),
            address(tokenB),
            liquidity,
    ...
    function testAddLiquidityRevertsWithoutApprovals() public {
        ...
        ammRouter.addLiquidity(
            address(tokenA), address(tokenB), amountA, amountB, amountA, amountB, lp, _validDeadline()
        );
        vm.stopPrank();
    }

    function testSwapTokensForExactTokensBasicSwap() public {
        ...
        _safeApprove(address(tokenA), address(ammRouter), type(uint256).max);
        _safeApprove(address(tokenB), address(ammRouter), type(uint256).max);
        ammRouter.addLiquidity(address(tokenA), address(tokenB), amountA, amountB, 0, 0, lp, _validDeadline());
        vm.stopPrank();
        ...
        uint256[] memory amounts =
            ammRouter.swapTokensForExactTokens(amountOut, type(uint256).max, path, trader, _validDeadline());
        vm.stopPrank();
        ...
        (uint256 reserveAAfter, uint256 reserveBAfter,) = ammRouter.getReserves(address(tokenA), address(tokenB));
    ...
    function testSwapTokensForExactTokensMultiHopSwapSwapsThroughPath() public {
        ...
        ammRouter.addLiquidity(address(tokenB), address(tokenC), amountB, amountC, 0, 0, lp, _validDeadline());
        vm.stopPrank();
    ...
    function testSwapTokensForExactTokensRevertsOnExcessiveInputAmount() public {
        // Arrange: Seed liquidity
        uint256 amountA = 10e18;
        uint256 amountB = 10e18;
        ...
        ammRouter.swapTokensForExactTokens(amountOut, amountInMax, path, trader, _validDeadline());
        vm.stopPrank();
    }
    ...
    function testSwapTokensForExactTokensRevertsOnDeadlineExpired() public {
        ...
        vm.expectRevert(bytes("DEADLINE_EXPIRED"));
        ammRouter.swapTokensForExactTokens(amountOut, type(uint256).max, path, trader, _invalidDeadline());
        vm.stopPrank();
    }
    ...
    function testSwapTokensForExactTokensRespectsToAddress() public {
        ...
        vm.startPrank(trader);
        _safeApprove(address(tokenA), address(ammRouter), type(uint256).max);
        ammRouter.swapTokensForExactTokens(amountOut, type(uint256).max, path, recipient, _validDeadline());
        vm.stopPrank();
