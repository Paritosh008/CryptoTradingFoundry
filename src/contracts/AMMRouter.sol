...
contract AMMRouter is IAMMRouter {
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path) public view returns (uint256[] memory amounts) {
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            amounts[i + 1] = AMMLibrary.getAmountOut(amounts[i], reserveIn, reserveOut);
        }
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path) public view returns (uint256[] memory amounts) {
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = AMMLibrary.getAmountIn(amounts[i], reserveIn, reserveOut);
        }
    }
    ...
    function addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB, address pair) = getReserves(tokenA, tokenB);
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                ...
                require(amountAOptimal <= amountADesired, "EXCESSIVE_A_OPTIMAL_AMOUNT");
                require(amountAOptimal >= amountAMin, "INSUFFICIENT_A_AMOUNT");
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
        }
        // Step 4: Transfer tokens from user to pair
        AMMLibrary.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        AMMLibrary.safeTransferFrom(tokenB, msg.sender, pair, amountB);
    ...
    function removeLiquidity(
        ...
        address pair = AMMLibrary.pairFor(factory, tokenA, tokenB, initCodeHash);
        AMMLibrary.safeTransferFrom(pair, msg.sender, pair, liquidity);
        ...
        (address _tokenA,) = AMMLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == _tokenA ? (amount0, amount1) : (amount1, amount0);
    ...
    //[0xa,0xb,0xc] [100,200,300]
    //amountOut = 200

    // User sends 100 of 0xa → pair(0xa, 0xb)
    //                                 ↓
    //                 pair(0xa, 0xb) swaps: receives 100 of 0xa, sends 200 of 0xb → pair(0xb, 0xc)
    //                                                                                     ↓
    //                                     pair(0xb, 0xc) swaps: receives 200 of 0xb, sends 300 of 0xc → _to
    function _swap(uint256[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint256 i = 0; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);

            (address tokenA,) = AMMLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];

            (uint256 amountAOut, uint256 amountBOut) =
                input == tokenA ? (uint256(0), amountOut) : (amountOut, uint256(0));

            address to = i < path.length - 2 ? AMMLibrary.pairFor(factory, output, path[i + 2], initCodeHash) : _to;

            ITokenPair(AMMLibrary.pairFor(factory, input, output, initCodeHash)).swap(amountAOut, amountBOut, to);
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        AMMLibrary.safeTransferFrom(
            path[0], msg.sender, AMMLibrary.pairFor(factory, path[0], path[1], initCodeHash), amounts[0]
        );
